...
export const CartDrawer: React.FC = () => {
  ...
  return (
    <>
      {/* Drawer UI, rendered only when isCartOpen is true */}
      {isCartOpen && (
        <>
          {/* Backdrop */}
          <div 
            className="fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-[4px] transition-all duration-300"
            onClick={() => setIsCartOpen(false)}
          />

          {/* Sliding Drawer Container */}
          <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-md bg-white border-l border-slate-100 shadow-2xl flex flex-col h-full animate-slide-in">
            {/* Header */}
