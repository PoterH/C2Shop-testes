...
export default async function handler(req, res) {
    ...
    try {
        ...
        const isApproved = ['approved', 'paid', 'completed', 'confirmed', 'pago', 'aprovado', 'autorizado', 'pedido_pago', 'pedido_aprovado'].includes(status) ||
        ...
        const email = payload.customer?.email || data.customer?.email ||
        ...
        if (matchedProducts.length === 0 && productSku) {
            const matched = products.find((p) => p.slug === productSku ||
