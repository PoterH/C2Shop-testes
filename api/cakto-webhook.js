...
export default async function handler(req, res) {
    ...
    try {
        ...
        const isApproved = ['approved', 'paid', 'completed', 'paid_out', 'confirmed', 'purchase_approved'].includes(status.toLowerCase()) ||
        ...
        const email = payload.buyer?.email || payload.customer?.email || payload.email || payload.payer?.email ||
            data.buyer?.email || data.customer?.email || data.email || data.payer?.email;
        const name = payload.buyer?.name || payload.customer?.name || payload.name || payload.payer?.name ||
        ...
        const productSlug = payload.product?.slug || payload.product?.id || payload.product_id || payload.items?.[0]?.id || payload.offer?.product_id ||
            data.product?.slug || data.product?.id || data.product_id || data.items?.[0]?.id || data.offer?.product_id;
        const orderId = payload.id || payload.order_id || payload.reference ||
        ...
        if (itemsArray.length > 0) {
            for (const item of itemsArray) {
                ...
                const matched = products.find((p) => p.slug === sku ||
                    p.id === sku ||
